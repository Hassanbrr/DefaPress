...
namespace DefaPress.Application.Services.Implements
{
    public class ArticleCategoryService : IArticleCategoryService
    {
        ...
        {
            var category = await _unitOffWork.ArticleCategoryRepository.GetByIdAsync(id, null, cancellationToken);
        ...
        private async Task<bool> CheckIfCategoryHasArticles(int categoryId, CancellationToken cancellationToken = default)
        {
            try
            {
                 ...
                 var articles = await _unitOffWork.ArticleRepository.FindAsync(a => a.ArticleCategoryId == categoryId);
                return articles.Any();
        ...
        private async Task<bool> WouldCreateCircularReference(int categoryId, int? newParentId, CancellationToken cancellationToken)
        {
            ...
            while (currentParentId != 0)
            {
                ...
                var parent = await _unitOffWork.ArticleCategoryRepository.GetByIdAsync(currentParentId, null, cancellationToken);
        ...
        public async Task UpdateDisplayOrdersAsync(IEnumerable<(int CategoryId, int DisplayOrder)> orders, CancellationToken cancellationToken = default)
        {
            ...
            {
                var category = await _unitOffWork.ArticleCategoryRepository.GetByIdAsync(categoryId, null, cancellationToken);
                if (category != null)
                {
